  ...
  return (
    <div className={`h-lvh w-lvw relative flex ${theme}`}>
      <div className='h-full  flex flex-col pb-3 pl-3 justify-end gap-4 z-10 pointer-events-none **:pointer-events-auto font'>
        ...
        >
          <MdZoomOutMap className={`absolute ${clean ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}/>
          <MdZoomInMap className={`absolute ${!clean ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}/>
        </button>
      ...
      <AnimatePresence>
        {!clean && (
          <motion.div
            ...
            className='h-full md:h-[calc(100%-5rem)] w-full flex flex-col justify-between gap-4 absolute z-10 pointer-events-none p-8 pl-20'
          >
            <Header />
            <Outlet />
          </motion.div>
